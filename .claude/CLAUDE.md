...
Squads são grupos de 5 agentes especializados em domínios específicos (não fazem parte do core AIOX).

| Squad | Agentes | Domínio | Ativação |
|-------|---------|---------|----------|
| `visual-design-squad` | @visual-designer, @ux-interaction-designer, @frontend-developer-squad, @responsive-specialist, @performance-a11y-specialist | Design visual, CSS, responsividade, a11y | `@visual-designer` → `*design-visual` |
| `marketing-instagram-squad` | @profile-strategist, @copywriter, @content-planner, @trend-researcher, @analytics-agent | Instagram orgânico, conteúdo, estratégia, analytics | `@profile-strategist` → `*define-strategy` |

**Validação:** Use `@squad-creator *analyze-squad squads/{squad-folder}` para checar integridade.  
**Guia completo:** `.claude/SQUAD-ACTIVATION.md`
...
- Squad agents are activated the same way: @visual-designer, @profile-strategist, etc.
...
### Squad Agent Critical Rule
⚠️ **NUNCA use `/Explore` para ativar squads** — dispara pesquisa web automática, custa ~160.9K tokens (2m 58s).  
✅ **USE:** `@agent-name` direto — custo ~3.5K tokens (15s, 40x mais rápido).
