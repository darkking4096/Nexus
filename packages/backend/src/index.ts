 ...
 * Async startup ensures PostgreSQL connection is ready before routes are registered
 */
async function startServer() {
  try {
    // Initialize PostgreSQL database connection pool first
    console.log('🔄 Initializing PostgreSQL connection pool...');
    const db = await initializePostgresDatabase();
    ...
    // Auth login rate limiter: 5 requests per 5 minutes per IP
